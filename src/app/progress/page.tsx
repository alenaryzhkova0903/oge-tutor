...
export default function ProgressPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const [{ data: attempts }, { data: tasks }] = await Promise.all([
        supabase.from('attempts').select('is_correct, task_id, created_at').eq('user_id', session.user.id),
        supabase.from('tasks').select('id, topic, oge_task_number'),
      ])
      ...
      ;(tasks as Pick<Task, 'id' | 'topic' | 'oge_task_number'>[]).forEach((t) => {
        topicMap[t.id] = t.topic
      ...
      ;(attempts as Pick<Attempt, 'is_correct' | 'task_id'>[]).forEach((a) => {
        total++
        if (a.is_correct) correct++
        const topic = topicMap[a.task_id]
        if (topic && acc[topic]) {
          acc[topic].total++
            ...
            return (
              <div key={r.topic} className={`border rounded-xl px-5 py-3 ${color}`}>
                <div className="flex justify-between items-center">
                  ...
                  <Link href={`/practice/topic/${r.topic}`} className="text-xs underline ml-3 shrink-0">
                    Тренировать
                  </Link>
                </div>
            ...
            <tbody>
              {stats.map((s, i) => (
                <tr key={s.topic} className={i < stats.length - 1 ? 'border-b' : ''}>
                  ...
                  <td className="px-4 py-3 text-right font-mono text-gray-600">{s.correct}/{s.total}</td>
